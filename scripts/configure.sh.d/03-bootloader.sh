...
if [[ $OSI_USE_ENCRYPTION == 1 ]]; then
	...
	echo "options rd.luks.name=$LUKS_UUID=$rootlabel root=/dev/mapper/$rootlabel $KERNEL_PARAM" | sudo tee -a $workdir/boot/loader/entries/sunny.conf || quit_on_err 'Failed to configure bootloader config'
	echo "options rd.luks.name=$LUKS_UUID=$rootlabel root=/dev/mapper/$rootlabel $KERNEL_PARAM" | sudo tee -a $workdir/boot/loader/entries/sunny-fallback.conf || quit_on_err 'Failed to configure bootloader fallback config'
	...
	echo "options root=\"LABEL=$rootlabel\" $KERNEL_PARAM" | sudo tee -a $workdir/boot/loader/entries/sunny.conf
	echo "options root=\"LABEL=$rootlabel\" $KERNEL_PARAM" | sudo tee -a $workdir/boot/loader/entries/sunny-fallback.conf
